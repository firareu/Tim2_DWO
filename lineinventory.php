<div class="col-xl-12">
    <div class="card shadow mb-4">
        ...
        <div class="card-header py-4 d-flex flex-row align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold text-primary">Jumlah Inventory Produk Berdasarkan Lokasi</h6>
        </div>
        <!-- Card Body -->
...
$a = 1;
$query_produk = mysqli_query($conn, "SELECT productid, SUM(Quantity) AS total_quantity FROM factinventory GROUP BY productid;");
$jumlah_produk = mysqli_num_rows($query_produk);
$chart_produk = "";
while ($row1 = mysqli_fetch_array($query_produk)) {
    if ($a < $jumlah_produk) {
        $chart_produk .= $row1['total_quantity'] . ",";
        $a++;
    } else {
        $chart_produk .= $row1['total_quantity'];
    }
